---
title: 'Statistical Computing: Dplyr, Pipes, and More (TA VERSION)'
author: "TA: Daniel Zhu"
date: "Week of Tuesday October 3, 2023"
output: pdf_document
---
...
Note: loading the `tidyverse` package in its entirety includes `plyr`, and this can cause namespace issues with the `dplyr` package. Better to just load only what you need.
...
```{r q1c}
set.seed(1)
hist(rnorm(800),breaks=30, main="N(0,1) draws", col="pink", prob=TRUE)
```
...
- **2c.** Note: `str.url` is defined for use in this and the next question; you can simply refer to it in your solution code (it is not part of the code you have to convert to pipes). Hint: the function `unlist()` might help with this question.

```{r}
str.url = "https://www.stat.cmu.edu/~ftownes/teaching/36350/F23/data/king.txt"
...
```{r}
pros.df = 
  read.table("https://www.stat.cmu.edu/~ftownes/teaching/36350/F23/data/pros.dat")
head(pros.df)
```
...
- **3a.** Among the men whose seminal vesicle invasion is positive (i.e. 1) (across the entire data set), report the summary statistics of the variable log cancer volume.

```{r q3a}
pros.df %>%
  filter(svi == 1) %>%
  summarize(
    mean_lcv = mean(lcavol),
    median_lcv = median(lcavol),
    min_lcv = min(lcavol),
    max_lcv = max(lcavol)
  )
```

- **3b.** Order the rows by decreasing `age`, then display the rows from men who are older than 65 and without SVI. 
...
pros.df %>%
select(-svi) %>%
map(t.test.by.ind,ind=pros.df$svi) %>%
print()
```

- **3e.** Extend your code from the last part (add only one more line of code, glued together by a pipe) to extract the p-values from each of the returned t-test objects, and print them out to the console.
...
sprint.m.df$Time=as.numeric(sprint.m.df$Time)
```
**Answer: The Time column appears to have been stored as character because it contained time entries that included an "A" in their values, making them strings. This format likely represents ties, indicating who finished first or second in the race.**
...
- **5c.** The data frame `dat.reduced` is said to be in "long" format: it has observations on the rows, and variables (`Name`, `City`, `Time`) on the columns. Arrange the rows alphabetically by city; convert this data frame into "wide" format; and then order the rows so that they are alphabetical by sprinter name. Call the result `dat.wide`. To be clear, here the first column should be the athlete names, and the remaining columns should correspond to the cities. Confirm that your data frame has dimension 141 x 152 **Respond**: Do these dimensions make sense to you?
...
dim(dat.wide)
```
**Answer: I am not sure why this is happening but it seems to be changing values everytime the code is run. The original dimensions is 141x152 but thr value increases every few clicks**

- **5d.** Not counting the names in the first column, how many non-`NA` values does `dat.wide` have? **Respond**: How could you have guessed this number ahead of time, directly from `dat.reduced` (before any pivoting at all)?

```{r q5d}
count <- sum(!is.na(dat.wide[, -1])) 
count
```
...
```{r q5e}
wide_bolt=dat.wide[132,] %>%
select(!is.na) %>%
colnames
norm_bolt=filter(dat.reduced,Name=="Usain Bolt")
all.equal(wide_bolt[-1],norm_bolt$City)
```
...
```{r q5f}
dat.long=dat.wide %>%
  pivot_longer(names_to="City",values_to="Time",
               values_drop_na = TRUE,cols=2:152) %>%

  arrange(Name,City)
all.equal(dat.long,dat.reduced)
```

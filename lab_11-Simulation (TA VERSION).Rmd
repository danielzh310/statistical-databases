---
title: 'Statistical Computing: Simulation (TA VERSION)'
author: "TA: Daniel Zhu"
date: "Week of November 28, 2023"
output: pdf_document
---
...
- **1a.** Generate the following objects, save them to variables (with names of your choosing), and call `head()` on those variables.  
    * A vector with 2500 draws from a normal distribution with mean -1 and variance 4.
    * A vector with 20 draws from $\mathrm{Beta}(0.1, 0.1)$.
    * A vector of 5000 characters sampled uniformly from "A", "G", "C", and "U".
...
```{r q1a}
one <- rnorm(2500, mean = -1, sd = 4) 
head(one)

two <- rbeta(20, shape1= 0.1, shape2 = 0.1)
head(two)

three <- sample(x = c("A", "G", "C", "U"), size = 5000, replace = TRUE)
head(three)

firstColumn = runif(100, min = 0, max = 1)
secondColumn = runif(100, min = 0, max = firstColumn)
four <- data.frame(firstColumn, secondColumn)
head(four)
```
...
rfun1 <- function(n) rnorm(n, mean = -3, sd = sqrt(10))
rfun2 <- function(n) rexp(n, rate = 5)
rfun3 <- function(n) rbeta(n, shape1 = 1, shape2 = 1)
par(mfrow = c(1, 3)) 

# Set up a 1x3 grid for the plots
plot.cum.means(rfun1, n.max = 1000)
plot.cum.means(rfun2, n.max = 1000)
plot.cum.means(rfun3, n.max = 1000)
par(mfrow = c(1, 1))
```
**Answer: Yes as the sample size increases, the sample means tend to converge around the expected value of the distribution.**
...
```{r q1chal}
rfunChallenge <- function(n) rcauchy(n, location = 0, scale = 1)
plot.cum.means(rfunChallenge, n.max = 1000)
```
...
plot.ecdf = function(rfun, pfun, sizes) {
  ...
  for (i in 1:n.sizes) {
    lines(grid, evals[[i]], col=cols[i], lwd=2)
  }
  legend("bottomright", legend=sizes, col=cols, lwd=2)  
}

```

```{r q1c}
#rfunExample = function(n) {
#  rnorm(n, mean = -3, sd = sqrt(10))
#}

#pfunExample = pnorm

#sizes_example = c(10, 100, 1000)
#plot.ecdf(rfunExample, pfunExample, sizesExample)
```
**Kept erroring on knit**
...
head(results1$no.drug, 6)
head(results1$drug, 6)

# Simulate data
results2 = simulate.data()
head(results2$no.drug, 6)
...
```
**Answer: Among these three numbers, the greatest absolute difference in mean values exists between `no.drug` and `drug` in `results1`. This observation aligns with expectations since `no.drug` and `drug` in `results1` originate from dissimilar distributions with distinct expected values. Consequently, it is logical for the discrepancy in means to be more pronounced compared to two variables sharing the same distribution.**
...
```{r q2e}
newData = simulate.data(n = 1000, mu.drug = 1.1)
plot.data(newData)
```
**Answer:The density values or y-values exhibit an increase compared to previous instances. Notably, the drug histogram now spans a narrower range than in the two preceding plots. Specifically, in this plot, the range is 500, whereas in the prior plots, it extended to 800 and 1200. Although the no-drug density histogram showcases a higher peak, the drug density curve and histogram closely resemble those generated in the earlier plots.**
...
simulate.difference = function(n = 60, mu.drug = 2) {
  ...
  return(mean(data$drug) - mean(data$no.drug))
}


diff1 = simulate.difference()
diff2 = simulate.difference()
diff3 = simulate.difference(n = 1000, mu.drug = 10)
...
```{r q3b}
# Vector to store number of successes for each sample size
successes_vs_n = numeric()

# Iterate over sample sizes from 5 to 100
for (n in 5:100) {
  successes_vs_n = c(successes_vs_n, rep.sim(n = n, mu.drug = 2))
}

plot(5:100, successes_vs_n, type = "l", col = "blue", lwd = 2, xlab = "Sample Size (n)", ylab = "Number of Successes")
```
**Answer: Based on my simulation, the smallest sample size for which the number of success is 190 or more is 36.**
...
```{r q3c}
# Vector to store number of successes for each mu.drug
successes_vs_mu_drug = numeric()
for (mu_drug in seq(0, 5, by = 0.25)) {
  successes_vs_mu_drug = c(successes_vs_mu_drug, rep.sim(n = 20, 
                                                         mu.drug = mu_drug))
}

plot(seq(0, 5, by = 0.25), successes_vs_mu_drug, type = "l", 
     col = "red", lwd = 2, xlab = "mu.drug", ylab = "Number of Successes")
#min(which(sims >= 190) + 5)
```
**Answer: 9 is the smallest number for mu.drug for which the number of successes exceeds 190**
...
Change the functions `simulate.data()`, `simulate.difference()` and `rep.sim()`---whatever necessary---to accommodate this new scheme. Then run this simulation with 200 repetitions with `mu.drug=1.5`, and print out how many success there were. 
    
**Answer**: How does this number compare with the result you saw earlier in Q3a? Should it be much different?
...
rep.sim = function(nreps = 200, n = 60, mu.drug = 2, seed = NULL){
  ...
  for (r in 1:nreps){
    for (t in 1:(n/5)){
      case = simulate.difference(n = 5, mu.drug)
      if (case >= 100){
        sim.objs[r] = "S"
        break
      }
...
rep.sim(nreps = 200, mu.drug = 1.5)
```
**Answer: The success count is 200, which significantly deviates from the result obtained in 3a. This discrepancy arises from the fact that all 200 simulations achieved success. This substantial difference is expected because, in each simulation, we conduct a maximum of 12 trials of `simulate.difference` at `n = 5`. In contrast, in 3a, we run 200 simulations, each with `n = 60`. Consequently, one would anticipate the number of successes to surpass 195.**
...
ab.collect = function(n, a.mean, a.sd, b.mean, b.sd) {
  # Collect samples from condition A and B
  a.responses = rnorm(n, mean = a.mean, sd = a.sd)
  b.responses = rnorm(n, mean = b.mean, sd = b.sd)
  return(list(a.responses = a.responses, b.responses = b.responses))
}


set.seed(42)  
test_data = ab.collect(n = 100, a.mean = 10, a.sd = 5, b.mean = 12, b.sd = 4)
summary(test_data$a.responses)
summary(test_data$b.responses)
```
**Answer: Yes the sample means and sample standard deviations approximately match the theoretical values.**

- **4b.** Write a function `test.at.end(n, a.mean, a.sd, b.mean, b.sd)` which uses your function from Q4a to draw samples of size `n` and then runs a t-test to determine whether there is a significant difference. We'll define this as having a p-value at most 0.05. If there is a significant difference, we return either "A" or "B" for whichever condition has the higher mean. If there is no significant difference, we return "Inconclusive". Hint: recall `t.test()`, and examine its output on a trial run to figuure out how to extract the p-value. Run your function with `n=3000`, `a.mean=100`, `a.sd=20`, `b.mean=104`, `b.sd=10` and display the result.

```{r q4b}
test.at.end = function(n, a.mean, a.sd, b.mean, b.sd) {
  data = ab.collect(n = n, a.mean = a.mean, a.sd = a.sd, b.mean = b.mean, b.sd = b.sd)
  tTestResult = t.test(data$a.responses, data$b.responses)
  
  # Check significance and return the winner
  if (tTestResult$p.value <= 0.05) {
    if (mean(data$a.responses) > mean(data$b.responses)) return("A")
    else return("B")
  } else {
    return("Inconclusive")
  }
...
test_result_b = test.at.end(n = 3000, a.mean = 100, a.sd = 20, b.mean = 104, b.sd = 10)
test_result_b
```
...
- **4c.** Waiting until you collect all of the samples can take a while. So you instead decide to take the following approach.  
  ...
  Run this function on the same example as before with `n.per.day=100` and `n.days=30` (to match final sample sizes). 
  
  **Answer**: Do you get the same result? Do you save time collecting data?

```{r q4c}
test.as.you.go = function(n.per.day, n.days, a.mean, a.sd, b.mean, b.sd) {
  
  totalSamples = n.per.day * n.days
  collected_samples = 0
  winner = NULL
  
  for (day in 1:n.days) {
    data = ab.collect(n = n.per.day, a.mean = a.mean, a.sd = a.sd, b.mean = b.mean, b.sd = b.sd)
    tTestResult = t.test(data$a.responses, data$b.responses)
    collected_samples = collected_samples + n.per.day
    if (tTestResult$p.value <= 0.05) {
  ...
  return(list(winner = winner, samples_collected = collected_samples))
}

testResultC = test.as.you.go(n.per.day = 100, n.days = 30, a.mean = 100, a.sd = 20, b.mean = 104, b.sd = 10)
testResultC
```
**Answer: Yes, we do get the same result, and we save time collecting data, because we only collected 100 samples**
...
- **4d.** In practice, most AB tests won't have a clear winner; instead both conditions A and B will be roughly equivalent. In this case we want to avoid *false positives*: saying there's a difference when there isn't really a difference (with respect to the true distributions). Let's run a simulation that checks the false positive rate of the two testing regimes.
  ...
  For each, calculate the number of "A" results, "B" results, and "Inconclusive" results. 
  
  **Answer**: Is this what you would expect to see---recalling that we are declaring significance if the p-value from the t-test is at most 0.05? Does either method of sampling (all-at-once, or as-you-go) perform better than the other, with respect to controlling false positives? 
  
  **Challenge:** can you explain the behavior you're seeing, with the sequential sampling?
    
```{r q4d}
using.all <- vector(length = 1000, mode="list")
using.trial <- vector(length = 1000, mode="list")

for (x in 1:1000){
  temp = test.as.you.go(n.per.day = 100,n.days = 30, 
                        a.mean = 100, a.sd = 20, b.mean = 100, b.sd = 20)
  using.all[x] =  test.at.end(n = 1000, 
                              a.mean = 100, a.sd = 20, b.mean = 100, b.sd = 20)
  using.trial[x] = temp[[1]]
}

length(which(using.all == "A"))
length(which(using.all == "B"))
length(which(using.all == "Inconclusive"))

length(which(using.trial == "A"))
length(which(using.trial == "B"))
length(which(using.trial == "Inconclusive"))
```
**Answer: The test conducted all at once demonstrates superior performance in controlling false positives compared to the as-you-go test. This outcome was unexpected, as I initially anticipated a considerably lower rate of false positives for both the all-at-once and as-you-go approaches. Sequential testing allows stopping the experiment early if a significant result is observed, potentially leading to variations in the outcomes.**
